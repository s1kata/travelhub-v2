                    ...
                    <div class="tv-results-main" style="min-width:0">
                        <div class="tv-results-toolbar">
                            ...
                            <div class="tv-sort-rail">
                                ...
                                <button type="button" id="tv-compare-open-btn" class="tv-compare-open-btn" disabled>
                        ...
                        <div id="tv-compare-panel" class="tv-compare-panel hidden" aria-live="polite">
                            <div class="tv-compare-panel__head">
                                <h4>Сравнение туров</h4>
...
<<<<<<< HEAD
                <?php if ($homeShowcaseBootTours !== []): ?>
                    <?php foreach ($homeShowcaseBootTours as $tourCard): ?>
                        <?php
                        $tcHref = htmlspecialchars((string) ($tourCard['href'] ?? '/frontend/window/promotions.php'), ENT_QUOTES, 'UTF-8');
                        $tcImg = htmlspecialchars((string) ($tourCard['image'] ?? ''), ENT_QUOTES, 'UTF-8');
                        $tcHotel = htmlspecialchars((string) ($tourCard['hotelName'] ?? ''), ENT_QUOTES, 'UTF-8');
                        $tcCountry = htmlspecialchars((string) ($tourCard['countryName'] ?? ''), ENT_QUOTES, 'UTF-8');
                        $tcPrice = (int) ($tourCard['price'] ?? 0);
                        $tcNights = (int) ($tourCard['nights'] ?? 0);
                        $tcBadge = $tcPrice > 0 ? ('от ' . number_format($tcPrice, 0, '', ' ') . ' ₽') : 'Горящее';
                        $tcMetaParts = array_filter([
                            $tcCountry,
                            $tcNights > 0 ? ($tcNights . ' н.') : '',
                        ]);
                        $tcMeta = htmlspecialchars(implode(' · ', $tcMetaParts) ?: 'Горящий тур', ENT_QUOTES, 'UTF-8');
                        ?>
                        <a href="<?php echo $tcHref; ?>" class="th-tour-offer">
                            <div class="th-tour-offer__image" style="background-image:url('<?php echo $tcImg; ?>');"></div>
                            <div class="th-tour-offer__body">
                                <span class="th-tour-offer__badge"><?php echo htmlspecialchars($tcBadge, ENT_QUOTES, 'UTF-8'); ?></span>
                                <strong class="th-tour-offer__title"><?php echo $tcHotel; ?></strong>
                                <span class="th-tour-offer__meta"><?php echo $tcMeta; ?></span>
                            </div>
                        </a>
                    <?php endforeach; ?>
                <?php else: ?>
                    <?php for ($sk = 0; $sk < 4; $sk++): ?>
                        <div class="th-tour-offer th-tour-offer--skeleton" aria-hidden="true">
                            <div class="th-tour-offer__image"></div>
                            <div class="th-tour-offer__body">
                                <span class="th-tour-offer__badge">…</span>
                                <strong class="th-tour-offer__title">Загружаем туры</strong>
                                <span class="th-tour-offer__meta">актуальные цены</span>
                            </div>
                        </div>
                    <?php endfor; ?>
                <?php endif; ?>
...
<<<<<<< HEAD
                ...
                if (Array.isArray(homeShowcaseData.hot) && homeShowcaseData.hot.length) {
                    return homeShowcaseData.hot.slice(0, 24);
                }
...
>>>>>>> origin/master
            ...
            document.querySelectorAll('[data-tv-shift-days]').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    tvShiftSearchDates(parseInt(String(btn.getAttribute('data-tv-shift-days') || '0'), 10));
                });
            });
            ...
            if (tvCompareOpenBtn && tvComparePanel) {
                tvCompareOpenBtn.addEventListener('click', function() {
                    tvComparePanel.classList.toggle('hidden');
            ...
            var tvResContainerForCompare = document.getElementById('tv-search-results');
            if (tvResContainerForCompare) {
                tvResContainerForCompare.addEventListener('click', function(e) {
        ...
        function tvCompareRenderPanel() {
            ...
            if (!items.length) {
                list.innerHTML = '<p class="tv-compare-empty">Добавьте 2-3 тура для сравнения</p>';
                if (bestBox) {
            ...
            if (panel && panel.classList.contains('hidden') && count < 2) {
        ...
        function tvRenderPriceCalendar() {
            ...
            box.querySelectorAll('[data-tv-shift-days]').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    tvShiftSearchDates(parseInt(String(btn.getAttribute('data-tv-shift-days') || '0'), 10));
                });
            });
        ...
        function tvCompareToggleFromButton(btn) {
            ...
            if (tvCompareMap[key]) {
                ...
                if (tvCompareCount() >= TV_COMPARE_MAX) {
                    alert('Можно сравнить максимум 3 тура');
                    return;
                }
                tvCompareMap[key] = {
                    key: key,
        ...
        function tvShiftSearchDates(days) {
            ...
            var fp = (typeof tvDatePicker !== 'undefined' && tvDatePicker) ? tvDatePicker : (window.tvDatePicker || null);
            if (!fp || !fp.selectedDates || fp.selectedDates.length < 2) return;
            var from = new Date(fp.selectedDates[0]);
            var to = new Date(fp.selectedDates[1]);
            ...
            // Soft: без полного лоадера, сначала cache/SWR
