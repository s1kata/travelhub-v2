...
function overflowCheck() {
  ...
  // visible clipped text in header
  ...
  for (const [vpName, size] of [['desktop',{width:1280,height:800}], ['mobile',{width:390,height:844}]]) {
    ...
    for (const [name, url] of pages) {
      ...
      try {
        ...
        // dismiss geo/lead overlays if present
        await page.evaluate(() => {
          document.querySelectorAll('[class*=geo] button, .th-coral-popup__close, .th-sf-overlay .close, [data-dismiss], button').forEach(b => {
            const t = (b.textContent||'').trim();
            if (/Закрыть|Позже|Нет|×|X/.test(t) || b.getAttribute('aria-label')==='Закрыть') {
              try { b.click(); } catch(e){}
            }
          });
        });
        ...
        if (name==='home' && vpName==='desktop') {
          for (const label of ['Даты вылета', 'Ночей', 'Туристы', 'Фильтры']) {
            ...
            const vis = await page.evaluate(() => {
              const nodes = [...document.querySelectorAll('.th-coral-popup, .tv-filters-modal, .th-sheet, .flatpickr-calendar')];
            ...
            await page.evaluate(() => {
              document.querySelectorAll('[data-tv-tourists-close], .th-coral-popup__close, button').forEach(b=>{
                if (/Закрыть|Применить|×/.test(b.textContent||'') || b.classList.contains('th-coral-popup__close')) try{b.click()}catch(e){}
              });
            });
            await page.waitForTimeout(200);
          }
          // hotels tab
          await page.evaluate(() => {
            const t = [...document.querySelectorAll('[role=tab],button,a')].find(x => /^\s*Отели\s*$/.test((x.textContent||'').trim()) && x.closest('.tv-search, .th-search, form, .search'));
            (t || [...document.querySelectorAll('[role=tab]')].find(x=>/Отели/.test(x.textContent||'')))?.click();
          });
          await page.waitForTimeout(500);
          item.hotelsTab = await page.evaluate(() => {
            const sel = document.querySelector('[role=tab][aria-selected=true], .is-active, .active');
            return {selected: (sel&&sel.textContent||'').trim().slice(0,40), fields: [...document.querySelectorAll('button[aria-current], .tv-search-field, .th-search-field')].slice(0,8).map(b=>(b.textContent||'').trim().replace(/\s+/g,' ').slice(0,40))};
          });
